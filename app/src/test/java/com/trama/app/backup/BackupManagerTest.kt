...
import com.trama.app.backup.BackupManager.Backup
import com.trama.app.backup.BackupManager.BackupEntry
import com.trama.app.backup.BackupManager.BackupRecording
...
class BackupManagerTest {
    ...
    fun `getBackupFileName returns string matching expected pattern`() {
        ...
        assertTrue(fileName.startsWith("trama-backup-"))
        assertTrue(fileName.endsWith(".json"))
        // Pattern: trama-backup-YYYY-MM-DD.json
        assertTrue(fileName.matches(Regex("trama-backup-\\d{4}-\\d{2}-\\d{2}\\.json")))
    }
}
